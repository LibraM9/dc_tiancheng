...
def w2v_feat(data_frame, feat, mode):
    ...
    for m, t in enumerate(feat):
        ...
        tmp_df = data_frame[['UID', t]].merge(tmp_df, on=t)#单词转为向量
        tmp_df = tmp_df.drop_duplicates().groupby('UID').agg(stat_list).reset_index()#对向量进行聚合，求各种聚合指标
